...
class FC(Layer):
    ...
    def forward(self, inputs):
        ...
        """
        self.inputs = inputs
        return (np.dot(inputs, self.weights.T) + self.bias).reshape((inputs.shape[0], self.num_outputs))
    ...
    def backward_inputs(self, grads):
        ...
        """
        return np.dot(grads, self.weights).reshape((grads.shape[0], self.num_inputs))
    ...
    def backward_params(self, grads):
        ...
        """
        grad_weights = np.dot(grads.T, self.inputs).reshape(self.weights.shape)
...
class ReLU(Layer):
    ...
    def backward_inputs(self, grads):
        ...
        Returns:
          ndarray of shape (N, C, H, W).
        """
        return (self.inputs > 0) * grads


class SoftmaxCrossEntropyWithLogits():
    def __init__(self):
        self.has_params = False
        self.softmax = lambda x: np.exp(x) / np.sum(np.exp(x), axis=1).reshape(50, 1)
    ...
    def forward(self, x, y):
        ...
        return np.average(np.sum(-y * np.log(softmax), axis=1))
...
class L2Regularizer():
    ...
    def forward(self):
        ...
        """
        # TODO
        return self.weight_decay * (np.sum(self.weights * self.weights) * 0.5)
